...
const EngineeringDashboard = () => {
  ...
  // Load JIRA data from local JSON file (generated by fetch_jira_data.py)
  useEffect(() => {
    const loadJiraData = async () => {
      try {
        const response = await fetch('/jira-data.json');
        if (!response.ok) {
          console.log('[JIRA] No jira-data.json found. Run: python fetch_jira_data.py');
          return;
        }
        const data = await response.json();
        setJiraTicketInfo(data.tickets || {});
        console.log(`[JIRA] Loaded ${Object.keys(data.tickets || {}).length} tickets from local file`);
        if (data.lastUpdated) {
          console.log(`[JIRA] Data last updated: ${data.lastUpdated}`);
        }
      } catch (error) {
        console.error('[JIRA] Error loading jira-data.json:', error);
      }
    };
  ...
  const getEngineerWorkBreakdown = (contributors) => {
    // Get per-engineer breakdown of work types
    const engineerData = [];
    ...
    contributors.forEach(contributor => {
      ...
      // Only include engineers with PRs
      const totalPRs = Object.values(typeCounts).reduce((sum, count) => sum + count, 0);
      if (totalPRs > 0) {
        engineerData.push({
    ...
    return engineerData.sort((a, b) => b.total - a.total);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
                    ...
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 max-w-md">
                        <span className="text-sm text-gray-700 line-clamp-2">
                          {ticketInfo ? ticketInfo.summary : (
                            <span className="text-gray-400 italic">Run: python fetch_jira_data.py</span>
                          )}
                        </span>
                      </td>
      ...
      {/* Engineer Work Breakdown */}
      {data.contributors.length > 0 && (() => {
        const engineerWorkData = getEngineerWorkBreakdown(data.contributors);

        if (engineerWorkData.length === 0) return null;
        ...
        return (
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              Work Breakdown by Engineer
            </h2>
            <p className="text-sm text-gray-600 mb-4">
              Distribution of features, bugs, and other work types per engineer
            </p>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart
                data={engineerWorkData}
              ...
              >
                    ...
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-white p-3 border border-gray-200 rounded shadow-lg">
                          ...
                          <p className="text-sm text-gray-600 mb-2">Total: {data.total} PRs</p>
                ...
                />
                <Legend />
                <Bar dataKey="feat" stackId="a" fill={COLORS.feat} name="Features" />
                <Bar dataKey="fix" stackId="a" fill={COLORS.fix} name="Bugs" />
                <Bar dataKey="chore" stackId="a" fill={COLORS.chore} name="Chores" />
                <Bar dataKey="docs" stackId="a" fill={COLORS.docs} name="Docs" />
                <Bar dataKey="refactor" stackId="a" fill={COLORS.refactor} name="Refactor" />
                <Bar dataKey="test" stackId="a" fill={COLORS.test} name="Tests" />
                <Bar dataKey="style" stackId="a" fill={COLORS.style} name="Style" />
                <Bar dataKey="perf" stackId="a" fill={COLORS.perf} name="Performance" />
                <Bar dataKey="ci" stackId="a" fill={COLORS.ci} name="CI/CD" />
